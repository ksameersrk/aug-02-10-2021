...
var inputRowMap map[int][]int = make(map[int][]int) //Not really useful in this context
...
func splitRows() {
	rows := strings.Split(strings.TrimSpace(sampleInput2), "\n")
	for index, row := range rows {
		rowNums := strings.Fields(row)
		rowArr := make([]int, 0)
		for _, numStr := range rowNums {
			num, _ := strconv.Atoi(numStr)
			rowArr = append(rowArr, num)
		}
		inputRowMap[index] = rowArr
	}
}
